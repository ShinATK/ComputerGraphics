...
int main()
{
    ...
    // 使用 glfwWindowHint 来配置GLFW
    // 第一个参数：选项名称，以GLFW_开头的枚举值中选择
    // 第二个参数：整型值
    // 下边是告诉GLFW这里使用的OpenGL版本是4.5
    ...
    // 参数：前两个控制窗口左下角位置，后两个为渲染窗口的宽高（像素），可以小于GLFW中的设置
    ...
    // 每次循环开始前检查GLFW是否被要求退出，返回true则认为渲染结束
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
        // 按下则将WindowShouldClose属性设置为true
