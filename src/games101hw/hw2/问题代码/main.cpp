...
Eigen::Matrix4f get_projection_matrix(float eye_fov, float aspect_ratio, float zNear, float zFar)
{
    ...
    float near = zNear;
    float far = zFar;
    float hal_eye_fov = 0.5 * eye_fov / 180.0 * MY_PI;
    float top = near * tan(hal_eye_fov);
    float bottom = -top;
    float right = top * aspect_ratio;
    float left = -right;

    Eigen::Matrix4f orth_translate = Eigen::Matrix4f::Identity();
    orth_translate << 1, 0, 0, -(right + left) / 2,
        0, 1, 0, -(top + bottom) / 2,
        0, 0, 1, -(near + far) / 2,
        0, 0, 0, 1;
    ...
    projection = orth_scale * orth_translate * persp_to_orth;
